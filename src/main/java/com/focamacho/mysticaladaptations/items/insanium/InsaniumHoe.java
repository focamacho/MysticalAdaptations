...
public class InsaniumHoe extends ItemHoe implements IHasModel {
	...
	public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced){
		int range = (getRange(stack) == 2 ? 5 : 7);
		tooltip.add(Tooltips.HOE_TOOLTIP[0] + " " + Colors.GRAY + Minecraft.getMinecraft().gameSettings.keyBindSneak.getDisplayName() +  " " + Tooltips.HOE_TOOLTIP[1] + " " + Colors.DARK_PURPLE + range + "x" + range + Colors.GRAY + ".");
		tooltip.add(Tooltips.DURABILITY + Colors.DARK_PURPLE + Tooltips.UNLIMITED);
	...
	public int getRange(ItemStack stack){
		if(stack.getItem() == ModItems.INSANIUM_HOE){
        	NBTTagCompound tag = NBTHelper.getTagCompound(stack);
        	if(tag.hasKey(ToolType.TOOL_TYPE)){
